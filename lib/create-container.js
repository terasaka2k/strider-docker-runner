...
function findLocalImage(docker, image, done) {
  ...
  docker.listImages({ filter: withoutTag }, function(err, images) {
    ...
    images.some(function(img) {
      if (~img.RepoTags.indexOf(fullname)) {
        found = img;
        return true;
      }
    });
...
function create(createOptions, docker, config, done) {
  docker.createContainer(createOptions, function(err, container) {
    ...
    debug('Creaated a container container', container.id);
    ...
    function attached(err, streamc) {
      ...
      container.start({
        ...
        if(err) {
          debug('Error while starting container', config, err);
          kill(function(err) {
            debug('Error while killing the container, which failed to start', err);
          });
          return done(new Error(err));
        }
        debug('Started the container', container.id, data);

        container.wait(function(err, data) {
          debug('Done with the container', container.id, err, data);
          container.stop(function(err, _) {
            debug('Stopped the container', container.id);
          });
        });
    ...
    function spawn(streamc, command, args, options, done) {
      ...
      debug('Running command', container.id, command, args);
...
module.exports = function(createOptions, docker, config, done) {
  async.waterfall([
    function(callback) {
      ...
      if (localImage) {
        debug('Container image is already locally', localImage.id);
        return callback();
      }
